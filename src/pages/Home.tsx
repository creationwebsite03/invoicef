    ...
    <div className="overflow-hidden">
      ...
      <header className="pt-20 pb-24 px-8 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-12 gap-16 items-center">
          ...
          >
            <h1 className="text-5xl sm:text-6xl md:text-8xl font-headline font-extrabold tracking-tighter leading-[0.95] text-zinc-900 mb-8 text-balance">
              {t("Create Professional")} <span className="block mt-2">{t("Invoice in 30 Seconds")}</span>
            </h1>
            <p className="text-xl md:text-2xl text-zinc-500 max-w-xl mb-12 leading-relaxed font-medium">
              {t("Free invoice generator used by freelancers and businesses worldwide.")}
            </p>
            <div className="flex flex-wrap gap-6">
              <Link
                to="/tools/invoice"
                className="px-10 py-5 bg-zinc-900 text-white rounded-full font-black text-xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-zinc-900/10"
              >
                {t("Create Invoice")}
              </Link>
              <Link
                to="/tools"
                className="px-10 py-5 bg-white border-2 border-zinc-100 text-on-surface rounded-full font-black text-xl hover:bg-zinc-50 transition-all shadow-lg shadow-zinc-200/50"
              >
                {t("Explore Tools")}
              </Link>
            </div>
      ...
      {/* Trust Section */}
      <section className="py-24 bg-surface-container-low border-b border-zinc-100">
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <motion.div
                ...
                className="bg-white p-10 rounded-3xl shadow-sm border border-outline-variant/10 flex flex-col items-center text-center hover:shadow-xl hover:shadow-zinc-500/5 transition-all duration-300"
              >
                <span className="text-5xl font-bold text-zinc-900 mb-3 tracking-tighter">{stat.value}</span>
      ...
      <section className="py-24 max-w-7xl mx-auto px-8">
        <div className="mb-20 text-center md:text-left">
          <h2 className="text-4xl md:text-5xl font-headline font-bold text-zinc-900 mb-6 tracking-tighter">{t("Popular Billing Tools")}</h2>
          <p className="text-zinc-500 text-lg max-w-2xl leading-relaxed">{t("Access our curated directory of high-precision tools designed to streamline your billing and taxes.")}</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {TOOLS.map((tool, i) => (
            <motion.div
              ...
              className="group bg-white p-8 rounded-3xl border border-zinc-200 hover:border-zinc-300 transition-all duration-300 flex flex-col hover:shadow-xl hover:shadow-zinc-500/5"
            >
              <div className="w-14 h-14 bg-zinc-50 flex items-center justify-center rounded-2xl mb-8 group-hover:bg-zinc-900 group-hover:text-white transition-all duration-300">
                <tool.icon size={28} />
              </div>
              <h3 className="text-2xl font-bold mb-4 font-headline text-zinc-900">{t(tool.title)}</h3>
              <p className="text-zinc-500 mb-8 flex-grow leading-relaxed">{t(tool.desc)}</p>
              <Link to={tool.link} className="flex items-center gap-2 font-bold text-zinc-900 group-hover:gap-4 transition-all">
                {t("Open Tool")} <ArrowRight size={20} />
              </Link>
            </motion.div>
          ))}
        </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-8">
        <div className="text-center mb-20">
          <h2 className="text-4xl font-headline font-extrabold text-primary mb-4">{t("How it works")}</h2>
          <p className="text-on-surface-variant text-lg">{t("Three simple steps to professional billing.")}</p>
        </div>
      ...
      <section className="bg-zinc-50 py-24 border-t border-zinc-100">
        <div className="max-w-4xl mx-auto px-8 space-y-16">
          <div className="prose prose-zinc prose-sm max-w-none">
            <h2 className="text-3xl font-bold font-headline tracking-tighter mb-8 text-zinc-900">{t("World's Most Advanced Free Invoice Generator")}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-4">
              <h3 className="text-xl font-bold font-headline text-zinc-900">{t("Free Invoice Templates 2026")}</h3>
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold font-headline text-zinc-900">{t("GST & VAT Tax Compliance")}</h3>
              <p className="text-sm text-zinc-500 leading-relaxed">
          ...
          <div className="pt-16 border-t border-zinc-200">
            <h2 className="text-2xl font-black font-headline tracking-tighter mb-10 text-zinc-900">{t("Frequently Asked Questions")}</h2>
            <div className="space-y-8">
              <div>
                <h4 className="font-bold text-zinc-900 mb-2">{t("Is INVOXA completely free to use?")}</h4>
                <p className="text-sm text-zinc-600 leading-relaxed">Yes, INVOXA is a 100% <strong>free invoice generator</strong>. We provide unlimited access to our PDF creation tools, tax calculators, and digital signature features with no hidden costs and no subscription fees.</p>
              </div>
              <div>
                <h4 className="font-bold text-zinc-900 mb-2">{t("Can I download my invoices as PDF without an account?")}</h4>
                <p className="text-sm text-zinc-600 leading-relaxed">Absolutely. You can generate and download high-quality <strong>PDF invoices</strong> without ever creating an account. Your data is automatically saved to your browser's local storage for your convenience.</p>
              </div>
              <div>
                <h4 className="font-bold text-zinc-900 mb-2">{t("Does it support GST billing for India?")}</h4>
                <p className="text-sm text-zinc-600 leading-relaxed">Yes, INVOXA is fully optimized for Indian businesses. You can easily generate <strong>GST compliant invoices</strong>, including HSN codes and GSTIN details, using our specialized Indian business localization.</p>
              </div>
            </div>
          </div>
