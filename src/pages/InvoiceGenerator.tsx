  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const canvas = await (html2canvas as any)(targetRef.current, {
        scale: 2,
        useCORS: true,
        logging: true, // Enable logging to see potential warnings
        ...
        onclone: (clonedDoc) => {
          ...
          if (el) {
            ...
            // SMART COLOR CONVERSION: Dynamically convert oklch to rgb for PDF engine safety
            ...
            const convertColor = (val: string) => {
              if (!ctx || !val.includes('oklch')) return val;
              try {
                ...
                return val.includes('0 0 1') ? '#ffffff' : '#000000';
              }
            };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] antialiased pb-20">
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-10 py-10">
        {!loading && !user && (
          <div className="mb-8 p-4 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-3 text-zinc-400 select-none animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></div>
            <p className="text-xs font-black uppercase tracking-widest">{t("Log in to save your invoices and access cloud sync.")}</p>
          </div>
        )}

        <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-zinc-900 uppercase leading-none">
              {t(docType.type.charAt(0).toUpperCase() + docType.type.slice(1))} <span className="text-zinc-400">DESIGNER</span>
            </h1>
            <p className="text-sm font-medium text-zinc-500 mt-3 max-w-xl leading-relaxed">
              {t("Crafting a high-precision financial document for")} <span className="text-zinc-900 font-bold underline decoration-zinc-200 underline-offset-4">{invoice.clientDetails.name || t("Anonymous Client")}</span>. 
              {t(" Optimized for 100% calculation accuracy and international standards.")}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              ...
              className="lg:hidden flex-1 flex items-center justify-center gap-3 bg-zinc-100 text-zinc-900 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-zinc-200 transition-all active:scale-95 border border-zinc-200"
            >
              <Eye size={18} /> {t("Live Preview")}
            </button>
          </div>
        </header>
          ...
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!hasMinimalContent}
                className="p-2 bg-surface-container-highest text-on-surface rounded-lg shadow-sm hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
                title={hasMinimalContent ? t("Print") : t("Fill form to print")}
              >
                <Printer size={18} />
              </button>
            </div>
      ...
      {showPreviewModal && (
        <div className="fixed inset-0 z-[100] bg-zinc-900/80 backdrop-blur-sm flex items-center justify-center p-4 lg:hidden">
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 bg-zinc-50 flex justify-center min-h-0">
              <div className="origin-top transform scale-[0.35] sm:scale-[0.55] md:scale-[0.8] w-full flex justify-center p-0" data-preview-container>
            ...
            <div className="p-4 border-t border-zinc-100 flex gap-4">
              ...
              <button
                onClick={() => window.print()}
                className="flex-1 flex items-center justify-center gap-2 bg-zinc-100 text-zinc-900 py-3 rounded-xl font-bold hover:bg-zinc-200 transition-colors"
              >
      ...
      <div className="mt-20 border-t border-zinc-100 pt-16 max-w-4xl mx-auto space-y-12 pb-20">
        <section>
          <h2 className="text-3xl font-black font-headline tracking-tighter mb-6">{t("World's #1 Free Global Invoice Generator")}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <section className="space-y-4">
            <h3 className="text-xl font-bold font-headline">{t("Free Invoice Templates 2026")}</h3>
          ...
          <section className="space-y-4">
            <h3 className="text-xl font-bold font-headline">{t("Global Currency & Tax Support")}</h3>
            <p className="text-sm text-zinc-500 leading-relaxed">
        ...
        <section className="bg-zinc-50 p-8 rounded-3xl border border-zinc-100">
          <h2 className="text-2xl font-black tracking-tighter mb-8 font-headline">{t("Frequently Asked Questions (FAQ)")}</h2>
          <div className="space-y-8">
            <div>
              <h4 className="font-bold text-zinc-900 mb-2">{t("Is INVOXA really a free invoice generator?")}</h4>
              <p className="text-sm text-zinc-600">Yes, INVOXA provides unlimited <strong>free invoice creation</strong> with no hidden costs, no mandatory signups, and NO CAPTCHA for downloads. You can create, download, and share as many invoices as you need.</p>
            </div>
            <div>
              <h4 className="font-bold text-zinc-900 mb-2">{t("How do I save my invoices securely?")}</h4>
              <p className="text-sm text-zinc-600">INVOXA uses a secure <strong>Cloud Sync</strong> system. By creating a free account, all your invoices, client details, and business settings are instantly synced to our encrypted <strong>Firebase Firestore</strong> database, allowing you to access your billing data from any device in the world safely.</p>
            </div>
            <div>
              <h4 className="font-bold text-zinc-900 mb-2">{t("Can I generate a GST compliant invoice for India?")}</h4>
              <p className="text-sm text-zinc-600">Absolutely. By selecting India as your country, the system automatically enables <strong>GST invoice</strong> fields, allowing you to enter your Tax ID and calculate taxes according to Indian regulations with 100% accuracy.</p>
            </div>
          </div>
        </section>
